...
export function TourForm({ tour, isEditing, onSave, onCancel }: TourFormProps) {
  ...
  useEffect(() => {
    if (tour) {
      ...
      const transformedAdditionals =
        tour.additionals?.map((add: TourAdditional) => {
          const pricesObj: {
          ...
          if (add.prices && Array.isArray(add.prices)) {
            add.prices.forEach((p: TourPrice) => {
      ...
      setFormData({
        ...
        timelineImportantNote:
          tour.timelineImportantNote ||
          "Itinerario a modo informativo, puede variar de acuerdo a las condiciones climáticas y al grupo.",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const images: TourImage[] = cleanedFormData.images || [];
    const hasFeatured = images.some(
      (img: TourImage) => img.imageType === "FEATURED",
    );
    const hasHero = images.some((img: TourImage) => img.imageType === "HERO");
    ...
    if (cleanedFormData.timelineItems) {
      cleanedFormData.timelineItems = cleanedFormData.timelineItems.filter(
        (item: TimelineItem) =>
          item.timeLabel && item.title && item.description,
      );
    }
    ...
    if (cleanedFormData.testimonials) {
      ...
      cleanedFormData.testimonials.forEach(
        ...
        .map((item: TourAdditional): TourAdditional => {
          ...
          if (item.prices && !Array.isArray(item.prices)) {
            if (
              ...
              !isNaN(item.prices.ARS.adult) &&
              item.prices.ARS.adult >= 0
            ) {
              pricesArray.push({
                currency: "ARS",
                price: item.prices.ARS.adult,
              });
            }
            if (
              item.prices.USD &&
              typeof item.prices.USD.adult === "number" &&
              !isNaN(item.prices.USD.adult) &&
              item.prices.USD.adult >= 0
            ) {
              pricesArray.push({
                currency: "USD",
                price: item.prices.USD.adult,
              });
            }
          }
    ...
    const { id, departures, createdAt, updatedAt, ...dataToSave } =
      cleanedFormData;

    // Asegurar valores por defecto para campos no configurables
    dataToSave.timelineImportantNote =
      dataToSave.timelineImportantNote ||
      "Itinerario a modo informativo, puede variar de acuerdo a las condiciones climáticas y al grupo.";
    dataToSave.alternativeText =
      dataToSave.alternativeText || "Consultar precio";
      ...
      <div className={styles.tabContent}>
          ...
          <Card title="Información Básica">
              ...
              <div className={styles.checkbox}>
                <label>
                  <input
                    ...
                    onChange={(e) =>
                      updateField("allowsInfants", e.target.checked)
                    }
                    disabled={!isEditing}
                  />
            ...
            <div className={styles.restrictionsSection}>
              ...
              <ArrayFieldManager
                ...
                onDelete={(index) => {
                  const filtered = (formData.restrictions || []).filter(
                    (_, i: number) => i !== index,
                  );
            ...
            <div className={styles.additionalsSection}>
              <Card title="Opciones Adicionales">
                <p className={styles.sectionDescription}>
                  Agrega opciones adicionales que los usuarios pueden
                  seleccionar al hacer una reserva (ej: &quot;Con Canoas&quot;).
                </p>
                <ArrayFieldManager
                  title=""
                  ...
                  onDelete={(index) => {
                    const filtered = (formData.additionals || []).filter(
                      (_, i: number) => i !== index,
                    );
        ...
        {activeTab === "grid" && (
          <Card title="Tour Grid">
            <p className={styles.sectionDescription}>
              Estos campos afectan cómo se ve el tour en el grid/listado de
              tours.
            </p>
            <div className={styles.formGrid}>
              <ImagePicker
        ...
        {activeTab === "images" && (
          <div className={styles.imagesTab}>
            ...
            <GalleryManager
              ...
              tourSlug={
                formData.slug || generateSlug(formData.name || "") || "default"
              }
              disabled={!isEditing}
            />
          </div>
          ...
          <Card title="Contenido">
            ...
            <div className={styles.singleColumnLayout}>
              <section className={styles.relationSection}>
                <h3 className={styles.relationSectionTitle}>
